...
export class AppComponent implements OnInit {
      ...
      if (idtoken && role && accessToken && _reftoken) {
        try {
          ...
          if (token && resDec && actoken && reftoken) {
            ...
            this.getGenesysPremiumUser(decode)
            this.IdleLogout();
            this.getMstrProject();
            this.genesysSessionValidate();
  ...
  getGenesysPremiumUser(decode) {
    let getUser = localStorage.getItem("g_obj")
    let resRole = this.commonMethods.decryptData(localStorage.getItem("_&gr&"))
    if (getUser != null && resRole != null) {
      try {
        UserData.userName = JSON.parse(getUser)['userName'];
        UserData.email_Id = JSON.parse(getUser)['userEmail'];
        UserData.role = resRole.r;
      } catch (error) {
        UserData.userName = decode["custom:CustomerName"];
        UserData.email_Id = decode["email"];
      }
  ...
  genesysSessionValidate() {
    let genesysObj = localStorage.getItem('premium_app_auth_data');
    let gApi = localStorage.getItem('premium-app-successkpi:environment');
    if (genesysObj != null) {
      try {
        let loginobj = JSON.parse(genesysObj)
        loginobj.apiUrl = gApi;
        this.loginService.genesysSessionValidate(loginobj).subscribe(
          (data: any) => {
            console.log("session alive!!")
          },
          (error) => {
            console.log('session expired')
            localStorage.removeItem('premium_app_auth_data');
          }
        );
      } catch (error) {
        console.log(error)
